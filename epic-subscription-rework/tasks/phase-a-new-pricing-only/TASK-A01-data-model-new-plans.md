...
|-------------|-----|--------|
| ~~SPIKE-01: CloudPayments multi-month~~ | Внешняя | ✅ **Снят** — `Period=3/6/12, Interval=Month` подтверждено из документации API |
| Текущая схема БД | Внутренняя | Нужно изучить |

## Preconditions

- ✅ Подтверждено из документации API: CloudPayments поддерживает нужные интервалы (`Period=3/6/12, Interval=Month`)
